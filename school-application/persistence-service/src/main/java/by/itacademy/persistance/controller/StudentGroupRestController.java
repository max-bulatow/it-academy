...
@Tag(name = "student group", description = "Student group management APIs")
@Validated
@RestController
...
public class StudentGroupRestController {
    ...
    @Parameters(@Parameter(name = "id", required = true, description = "Student group Id", example = "1"))
    @GetMapping(path = "/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    public StudentGroupDto getByID(@Min(value = 1, message = "Id must be more than 0") final Integer id) {
        return studentGroupService.findById(id);
    }

    @ApiResponses({
            @ApiResponse(responseCode = "200", content = {
                    @Content(array = @ArraySchema(schema = @Schema(implementation = StudentGroupDto.class)),
                            mediaType = "application/json"
                    )
            }),
            @ApiResponse(responseCode = "400", content = {
    ...
    public Integer save(@RequestBody @Valid CreateStudentGroupDto dto) {
        return studentGroupService.save(dto);
    }
    ...
    public Integer update(@RequestBody @Valid StudentGroupDto dto) {
        return studentGroupService.update(dto);
    }
}
